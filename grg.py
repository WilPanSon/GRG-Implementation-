...
class GRG:
        ...
        for node in self.nodes:
            ...
            for child in node.adj:
                if not isinstance(child, Node):
                    raise TypeError(
                        f"Node {node} has an invalid child of type {type(child)}."
                    )
                if child not in self.nodes:
                    raise ValueError(
                        f"Node {node} connects to a child node that is not in the main GRG node set."
                    )
    ...
    def _has_cycles(self):
        ...
        visiting = (
            set()
        )  # Nodes currently in the recursion stack for the current DFS path
    ...
    def to_matrix(self):
        ...
        all_mutations = sorted(
            set(m for node in self.nodes for m in node.mut if m is not None)
        )
